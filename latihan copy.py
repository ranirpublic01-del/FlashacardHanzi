...
FILE_DATA = r"D:\01_Project\01_Coding\PY\flashcard\weather.json"
...
def next_card(event=None):
    ...
    info_label.config(
        text=f"Kartu {card_index + 1}/{len(cards)}"
    )
...
def toggle_answer(event=None):
    ...
    if showing_answer:
        ...
        info_label.config(
            text=f"Kartu {card_index}/{len(cards)}"
        )
...
root.bind("<Return>", next_card)

# Space = tampil/sembunyi jawaban
